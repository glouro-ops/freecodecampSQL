...
echo "$($PSQL "SELECT first_name, last_name, gpa FROM students WHERE gpa=4")"
...
echo "$($PSQL "SELECT course FROM students LEFT JOIN majors_courses USING (major_id) LEFT JOIN courses USING (course_id) GROUP BY course HAVING COUNT(students.student_id) = 1 ORDER BY course")"